...
//метод предиктор-корректор Адамса-Бэшфорта-Моултона
function res=method5(a,b,N,y0,k,step)
    NN = N*2^step;
    h=(b-a)/NN;
    x=a:h:b;
    y=x;
...
//    printf("\n");
    for i=5:length(x)
        ...
        x0 = x(i-1);
            ...
            yy = y0 + h*(55*f(x0,y0,k) - 59*f(x_1,y_1,k) + 37*f(x_2,y_2,k) - 9*f(x_3,y_3,k))/24;
            y1 = y0 + h*(9*f(x1,yy,k) + 19*f(x0,y0,k) - 5*f(x_1,y_1,k) + f(x_2,y_2,k))/24;
            x0 = x0 + h;
            y(i) = y1;
    end
    if step > 0 then
...
//    xtitle("Зависимость е от N в методе Эйлера","N","e");
    ...
    plot2d("ll",nn,ep,style=color('yellow'));
endfunction
...
filey = 'y.txt';
    ...
    for j=1:length(km)
        ...
        for i=1:10
            ...
            y1=method5(a,b,N,y0,k,0);
            y2=method5(a,b,N,y0,k,1);
            y3=method5(a,b,N,y0,k,2);
